...
class Planhat(object):
    ...
    def request(self, method, route, **kwargs):
        ...
        response = self._session.request(method, self._api_url + "/" + route, **kwargs)
        response.raise_for_status()
        return response

    def get_companies(self, segment_id=None):
        """https://docs.planhat.com/#companies"""
        return self.request("GET", "companies", params={"s": segment_id}).json()
    ...
    def get_contacts(self, company_id=None):
        return self.request("GET", "endusers", params={"c": company_id}).json()
    ...
    def get_licenses(self):
        return self.request("GET", "licenses").json()
